...
$(function() {
    var checking_tab_obj = {
        url: '../pjinfo/listsrefund?status=10,20,30,40,50,100,-100&refunding=1&type=checking',
        ...
        colModel: [
            ...
            { label: '已退保密金金额', name: 'sumRefund', index: 'sunRefund', width: 80 },
    ...
    var checked_tab_obj = {
        url: '../pjinfo/listsrefund?status=10,20,30,40,50,100,-100&finished=1&type=checked',
        ...
        colModel: [
            ...
            { label: '项目名称', name: 'name', index: 'name', width: 80 },
            { label: '标签', name: 'tabs', index: 'tabs', width: 150 },
            { label: '可退保密金金额', name: 'sumAmount', index: 'sumAmount', width: 80 },
            { label: '已退保密金金额', name: 'sumRefund', index: 'sumRefund', width: 80 },
            { label: '创建者', name: 'creator', index: 'creator', width: 80 },
            { label: '创建时间', name: 'createtime', index: 'createtime', width: 80 }
        ],
        viewrecords: true,
...
var vm = new Vue({
    ...
    methods: {
        ...
        // 查看退款按钮事件
        refund2: function(view) {
            var id = getSelectedRow("#jqGrid-" + view);
            if (id == null) {
                return;
            }
            // alert("你选中的数据的id是 : "+id);
            if (!vm_refund_win2.jqGrid_build) {
                var tab_objOrder = {
                    url: '../orderpay/listByProject',
                    pager: "#jqGridPagerRefund2",
                    gridComplete: function() {
                        // 隐藏grid底部滚动条
                        $("#jqGridRefund2").closest(".ui-jqgrid-bdiv").css({
                            "overflow-x": "hidden"
                        });
                    },
                    postData: {
                ...
                vm_refund_win2.show_win = true; // 要先show再build , display:none情况下build , 表格单元格将不能通过拖拉改变长度
                setTimeout(function() {
                    $("#jqGridRefund2").jqGrid(tab_objOrder);
                }, 16.7); // 设置一帧延迟 1000/60帧 = 16.7ms/帧 ,不延迟无法获取正确的弹窗宽度导致表格过窄(弹窗通过vue数据出现不适立即反应的);
                vm_refund_win2.jqGrid_build = true;
            } else {
                // var page = $("#jqGridRefund2").jqGrid('getGridParam', 'page');
                $("#jqGridRefund2").jqGrid('setGridParam', {
                    postData: {
                        "pjid": id,
                        "isrefund": 1,
                        "status": 10
