...
where[5] = new comefrom("广东","|广州|深圳|珠海|佛山|汕头|东莞|中山|韶关|江门|湛江|茂名|肇庆|惠州|梅州|汕尾|河源|阳江|清远|潮州|揭阳|云浮");
...
where[20] = new comefrom("河北","|石家庄|邯郸|邢台|保定|张家口|承德|廊坊|唐山|秦皇岛|沧州|衡水");
...
function initPC(oP,oC, sP, sC) {
	oProvince = oP;
	oCity=oC;
	var iProvinceIndex = 0;
	with (oProvince) {
		length = where.length;
		...
		oP.selectedIndex=iProvinceIndex;
		//options[selectedIndex].text = where[iProvinceIndex].loca;
		//options[selectedIndex].value = where[iProvinceIndex].loca;
	}
	var iCityIndex = 0;
	with (oCity) {
		loca3 = (where[iProvinceIndex].locacity).split("|");
		length = loca3.length;
		for (l = 0; l < length; l++) {
			if(loca3[l] == "城市名"){
				options[l].value = "";
		...
		oC.selectedIndex = iCityIndex;
		//options[selectedIndex].text = loca3[iCityIndex];
		//options[selectedIndex].value = loca3[iCityIndex];
	}
}