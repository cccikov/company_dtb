...
var vm = new Vue({
    ...
    methods: {
        getdata: function() {
            $.get("/m/paypage?orderid=" + orderid, function(r) {
        ...
        receiptsubmit: function(id){
            ...
            $.ajax({
                ...
                success: function(r) {
                    var result = eval('(' + r + ')');
                    location.href = "success.html?r="+r+"&type=receipt&id="+vm.pjInfo.id;
                }
            });
        }
    },
...
vm.getToken()
        ...
        function upload(basestr, type) {
            ...
            var orderid = vm.odOrder.id;
            var type = "";
            if(vm.odOrder.status == 0){
            	type = "BM"
            }else if(vm.odOrder.status == 10){
            	type = "DJ"
            }else if(vm.odOrder.status == 20){
            	type = "YK"
            }
            ...
            xhr.open('post', "/member/up?type=order/"+orderid+"&token=" + token);
